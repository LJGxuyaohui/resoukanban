...
- **第 1-2 页：知乎热榜** – 自动抓取知乎实时热点前 20 条并分页排版。作为桌面热搜看板，**方便随时获取第一手热门资讯，让你减少频繁打开手机的次数**。
...
3. 在代码顶部的**“用户自定义区”**，修改引号内的内容：
   ...
   - `ENABLED_PAGES`：控制显示的页面。如果不想要知乎页，可将 `"1,2,3,4"` 修改为 `"3,4"`。
...
等待约 1 分钟后，如果运行日志打出绿色的勾，你的墨水屏就会刷新并显示相应的画面。此后，它将按照设定的时间自动更新。

---

## 💖 致谢
- 天气数据支持：[高德开放平台](https://lbs.amap.com/) & [wttr.in](https://wttr.in)
- 热榜接口支持：[知乎](https://www.zhihu.com)
